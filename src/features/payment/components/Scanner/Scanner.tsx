...
function getMedianOfCodeErrors(decodedCodes: any) {
  // const errors = decodedCodes.filterny(x => x.error !== undefined).map(x => x.error);
...
const Scanner = ({
  ...
  const errorCheck = useCallback((result: any) => {
    if (!onDetected) {
      return;
    }
    const err: any = getMedianOfCodeErrors(result.codeResult.decodedCodes);
