...
export const updateTopUpData =
  (topUp?: TopUp) => (dispatch: Dispatch) => {
  ...
  (phoneNumber: string) => async (dispatch: Dispatch, getState: GetState) => {
    ...
    try {
      ...
      const { url, defaultHeaders, accountId, userId } =
        await getBaseRequestData("/TopUp/FindTopUpProductListByPhoneNumber", state);
      ...
      const response = await HttpClient.post<GetTopUpProductListByPhoneNumberResponse>(
        url,
        data,
        {
          headers: {
